...
def collect_samples(pid, queue, env, policy, custom_reward, mean_action,
					tensor, render, running_state, update_rs, min_batch_size,logger ,log_flag = False):
	torch.randn(pid, )
	log = dict()
	memory = Memory()
	num_steps = 0
	...
	while num_steps < min_batch_size:
		...
		obs1.append(state.reshape((1, -1)))#for storage
		...
		if log_flag:
			...
			data = np.concatenate((obs1, actions, obs2 - obs1, tmp1), axis=1)
...
class Agent:
	...
	def __init__(self, env_factory, policy, custom_reward=None, mean_action=True, render=False,
				 tensor_type=torch.DoubleTensor, running_state=None, num_threads=1, logger = None):
	...
	def collect_samples(self, min_batch_size):
		...
		for i in range(self.num_threads-1):
			worker_args = (i+1, queue, self.env_list[i + 1], self.policy, self.custom_reward, self.mean_action,
						   self.tensor, False, self.running_state, False, thread_batch_size,  self.logger , self.log_flag)
		...
		memory, log = collect_samples(0, None, self.env_list[0], self.policy, self.custom_reward, self.mean_action,
									  self.tensor, self.render, self.running_state, True, thread_batch_size ,self.logger ,log_flag = self.log_flag)
