...
class DiagGaussianPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        mean = tf.sigmoid(fc(latent_vector, 'pi', self.size, init_scale=init_scale, init_bias=init_bias) )
        
        
        
        logstd = tf.get_variable(name='logstd', shape=[1, self.size], initializer=tf.zeros_initializer() )  #tf.constant_initializer(-3)
